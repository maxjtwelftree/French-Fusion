classdef musicadded < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function createHomeScreen(app)
            ...
            % Music Player UI
            musicLabel = uilabel(app.HomeUI);
            musicLabel.Text = 'Study Music';
            musicLabel.FontSize = 14;
            musicLabel.FontWeight = 'bold';
            musicLabel.Position = [250, 190, 100, 20];
            musicLabel.HorizontalAlignment = 'center';
        
            playlistListBox = uilistbox(app.HomeUI);
            playlistListBox.Items = {'Song 1', 'Song 2', 'Song 3'};  % Replace with actual song titles
            playlistListBox.Position = [200, 150, 200, 100];
        
            playButton = uibutton(app.HomeUI, 'push');
            playButton.Position = [240, 130, 30, 30];
            playButton.Text = 'Play';
            playButton.ButtonPushedFcn = createCallbackFcn(app, @playButtonPushed, true);

            pauseButton = uibutton(app.HomeUI, 'push');
            pauseButton.Position = [280, 130, 30, 30];
            pauseButton.Text = 'Pause';
            pauseButton.ButtonPushedFcn = createCallbackFcn(app, @pauseButtonPushed, true);
        ...
        function playButtonPushed(app, ~)
            jsCode = "playAudio();";  % JavaScript function to play audio
            evaluateJavaScript(app.WebBrowser, jsCode);
        end
        
        function pauseButtonPushed(app, ~)
            jsCode = "pauseAudio();";  % JavaScript function to pause audio
            evaluateJavaScript(app.WebBrowser, jsCode);
        end
    end
        
    methods (Access = public)
        function app = musicadded
