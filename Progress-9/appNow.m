...
classdef appNow < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            ...
            app.isDarkMode = false; % Initialize as light mode by default
            app.StartTime = tic();
            app.Score = 0; % score being tracked for user functionality and so the game can be played with a repetition over time
            if app.isDarkMode
        ...
        function createHomeScreen(app)
            ...
            % Instead of two buttons, we can create a single button for toggling the theme.
            themeToggleButton = uibutton(app.HomeUI, 'push');
            
            % Check the current mode and set the appropriate emoji
            if app.isDarkMode
            ...
            % Set the ButtonPushedFcn to toggle the theme and also change the button's emoji.
            themeToggleButton.ButtonPushedFcn = @(~,~) toggleAndSetEmoji(app, themeToggleButton);
            
            % Set position (you can adjust if necessary)
            themeToggleButton.Position = [app.HomeUI.Position(3) - 40, app.HomeUI.Position(4) - 40, 30, 30];
            ...
            musicPanel.BackgroundColor = app.currentColors.bg; % Accessing property of the app object
            ...
            musicLabel.FontColor = app.currentColors.text; % Accessing property of the app object
            musicLabel.BackgroundColor = app.currentColors.bg; % Accessing property of the app object for label background too
        ...
        function switchToLightMode(app)
            app.HomeUI.Color = app.LightBackgroundColor;
            app.MusicPanel.BackgroundColor = app.LightBackgroundColor;
            musicLabel.BackgroundColor = app.LightBackgroundColor; % Update label background color
            musicLabel.BorderColor = app.LightBackgroundColor; % Update label border color
        end
        
        function switchToDarkMode(app)
            app.HomeUI.Color = app.DarkBackgroundColor;
            app.MusicPanel.BackgroundColor = app.DarkBackgroundColor;
            musicLabel.BackgroundColor = app.DarkBackgroundColor; % Update label background color
            musicLabel.BorderColor = app.DarkBackgroundColor; % Update label border color
        end
        ...
        function updateColors(app)
            ...
            app.HomeUI.Color = app.currentColors.bg;
            app.MusicPanel.BackgroundColor = app.currentColors.bg;
            % Update other UI elements' colors here
            
            musicLabel.BackgroundColor = app.currentColors.bg; % Update musicLabel's background color
        end

        function toggleAndSetEmoji(app, btn)
            toggleTheme(app); % Call the theme toggle
            
            % Change the button's emoji based on the current mode
            if app.isDarkMode
                btn.Text = '🌑';
            else
                btn.Text = '☀️';
            ...
            updateColors(app); % Add this line to update UI colors after theme toggle
        end
        ...
        function storyButtonPressed(app)
            ...
            exitButton = uibutton(app.UIFigure, 'push');
            exitButton.ButtonPushedFcn = createCallbackFcn(app, @exitButtonPushedStory, true);
            exitButton.Position = [app.UIFigure.Position(3) - 100, app.UIFigure.Position(4) - 40, 70, 30];  
            exitButton.FontColor = [1, 1, 1];
            exitButton.BackgroundColor = [0.3, 0.6, 1];
            exitButton.Text = 'Exit';
        ...
        function toggleTheme(app)
            ...
            % Check if other UIs exist and if they do, update their color
            if isfield(app, 'ChallengeUI') && isvalid(app.ChallengeUI)
                app.ChallengeUI.Color = app.currentColors.bg;
                % Add any other ChallengeUI components to adjust here if needed
            end
        
            % Repeat similar checks and updates for StoryUI, TranslatorUI, etc.
        end
...
function challengeButtonPushed(app, ~)    
        ...
        function exitChallenge(app)
            ...
            if app.FirstExit % If it's the first exit
                ...
                app.challengeButtonPushed(); % Navigate back to the challenge selection screen
            else
